...
import { type BlockMap, D1Blockstore, MST } from "../lib/mst";
...
import {
  assertRepoHead,
  bumpRoot,
  RepoBlobNotFoundError,
  RepoCommitConflictError,
} from "../db/repo";
...
import {
  assertBlobKeysAvailable,
  resolveRecordBlobKeys,
} from "./repo/blob-refs";
import {
...
export class RepoManager {
  ...
  private async getRootSnapshot(): Promise<
    { mst: MST; commitCid: string } | null
  > {
    ...
    return {
      mst: await MST.load(this.blockstore, mstRoot),
      commitCid: row.commitCid,
    };
  }
  ...
  private async getOrCreateRootSnapshot(): Promise<
    { mst: MST; commitCid: string | null }
  > {
    ...
    if (existing) {
      ...
      console.log(
        `[RepoManager] Loaded existing MST root: ${pointer.toString()}`,
      );
      return existing;
    }
  ...
  async addRecord(
    ...
    const { mst: currentMst, commitCid: expectedCommitCid } = await this
      .getOrCreateRootSnapshot();
    const prevMstRoot = expectedCommitCid === null
      ? null
      : await currentMst.getPointer();
  ...
  async createRecord(
    ...
    const {
      ...
      recordBlock,
      prevMstRoot,
      expectedCommitCid: rootCommitCid,
      newMstBlocks,
    ...
    const did = await this.getDid();
    const effectiveBlobKeys = blobKeys ??
      await resolveRecordBlobKeys(this.env, did, record);
    const uri = `at://${did}/${collection}/${key}`;

    const currentRoot = await mst.getPointer();
    const commitGuard = expectedCommitCid === undefined
      ? rootCommitCid
      : expectedCommitCid;
    const opsForCommit: RepoOp[] = [{
      action: "create",
      path: `${collection}/${key}`,
      cid: recordCid,
    }];
  ...
  async putRecord(
    ...
    const key = `${collection}/${rkey}`;
    const { mst: currentMst, commitCid: rootCommitCid } = await this
      .getOrCreateRootSnapshot();
    const prevMstRoot = rootCommitCid === null
      ? null
      : await currentMst.getPointer();
    ...
    if (existingCid?.toString() === recordCid.toString()) {
      if (typeof commitGuard !== "string") {
        ...
        if (repairResult.tag === "blobNotFound") {
          throw new RepoBlobNotFoundError();
        }
        if (repairResult.tag === "conflict") {
