...
describe("repo write pure constraints", () => {
  it("rejects forbidden raw record keys before lexicon conversion", () => {
    ...
    expect(() => parseRepositoryRecord("com.example.record", record)).toThrow(
  ...
  it("enforces strict ATProto datetime formats", () => {
    ...
    expect(() =>
      enforceRepoWriteLexiconConstraints(def("app.bsky.feed.post"), record)
    )
      .toThrow("datetime");
  });
  ...
  it("enforces blob accept and maxSize constraints", () => {
    ...
    expect(() =>
      enforceRepoWriteLexiconConstraints(
        def("app.bsky.actor.profile"),
        gifProfile,
      )
    )
  ...
  it("allows accepted blob constraints through nested unions", () => {
    ...
    expect(() =>
      enforceRepoWriteLexiconConstraints(def("app.bsky.feed.post"), post)
    ).not.toThrow();
  });
